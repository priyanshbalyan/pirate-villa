    ...
    <>
      {href && (text || Icon) && (
        <div className={twMerge('flex w-auto cursor-pointer', containerClass)}>
          {targetBlank ? (
            <Link
              className={twMerge('inline-flex items-center justify-center w-full sm:mb-0 dark:text-white dark:border-white ', linkClass)}
              href={href}
              target="_blank"
            ...
            <Link className={twMerge('inline-flex items-center justify-center w-full sm:mb-0 dark:text-white dark:border-white', linkClass)} href={href}>
              {Icon && <Icon className={twMerge(`w-5 h-5 ${text ? 'mr-1 rtl:mr-0 rtl:ml-1' : ''}`, iconClass)} />}
              {text}
            </Link>
          )}
