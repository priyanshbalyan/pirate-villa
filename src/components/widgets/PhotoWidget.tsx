...
export default function PhotoWidget({ north }: { north: boolean }) {
  ...
  return <>
    {tArray(north ? 'main_page_north_villa_image_grid' : 'main_page_south_villa_image_grid').map((text, index) => (
      <SafeImage
        key={index}
        className="shadow-lg h-[231px] w-[154px] md:h-[273px] md:w-[190px] md: bg-gray-400 dark:bg-slate-700 transition-transform ease-in-out duration-300 hover:scale-[1.01] object-cover"
        src={text}
        width={268}
        height={462}
